...
int main(){
    ...
    Person* s1 = new Student("Seb", 1884670);
    Person* i1 = new Instructor("Dr.V");
    c1.addPerson(s1);
    c1.addPerson(i1);

    std::vector<Person*> my_persons = c1.get_persons();
    std::cout << "Name of Student:" << my_persons[0]->get_name() << std::endl;
    std::cout << "Id of Student:" << static_cast<Student*>(my_persons[0])->get_id() << std::endl; // NR - Introducing students to casting
